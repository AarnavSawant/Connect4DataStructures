...
public class MiniMaxUtils {
    ...
    private static int getScore(TokenColor color,ArrayList<TokenColor> window) {
        ...
        for (int val = 0; val < window.size() - WINDOW_SIZE + 1; val++) {
            ...
            } else if (numTokensInWindow(window, WINDOW_SIZE, val, color) == 3 && numTokensInWindow(window, WINDOW_SIZE, val, TokenColor.NONE) == 1) {
                //If we can get 3 out of 4 and have a chance at winning, add THREE_PATTERN_REWARD
                score += THREE_PATTERN_REWARD;
            } else if (numTokensInWindow(window, WINDOW_SIZE, val, color) == 2 && numTokensInWindow(window, WINDOW_SIZE, val, TokenColor.NONE) == 2) {
                //If can get consecutive tokens add a smaller reard
                score += TWO_PATTERN_REWARD;
            }
            if (numTokensInWindow(window, WINDOW_SIZE, val, oppositeColor) == 3 && numTokensInWindow(window, WINDOW_SIZE, val, TokenColor.NONE) == 1) {
                //If there the opponent can win on the next move, take a penalty
                score -= THREE_OUT_OF_FOUR_PENALTY;
            }
        }

        //Checking for three-token trap (when there is three coins and two chances to win)
     ...
     * @return the count
     */
    public static int numTokensInWindow(ArrayList<TokenColor> window, int windowSize, int windowStart, TokenColor color) {
        int count = 0;
    ...
    public static Node miniMax(Board board, TokenColor color, int depth, boolean isMaximizing) throws FullColumnError {
       //Recursive Base Case: if we've reached the maximum depth of the Depth First Search, evaluate the score
        ...
        if (isMaximizing) {
            ...
            //We want to return the column that's going to maximize our score while minimizing opponents' score
